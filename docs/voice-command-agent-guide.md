...
主界面的「指令模式」默认关闭。用户主动打开后，RTC 持续把后续定型语音交给 TypeSafe 从已有指令候选中选择，直到用户再次关闭。普通转写不会调用 TypeSafe。

- 固定指令仍优先走本地精确匹配；
- 指令模式不会因执行完一句话自动关闭，用户再次点击开关才退出；
- 语义判断只能选择已有 `actions` 或 `aliases`，不能生成新动作；
- 置信度不足、模型失败或返回「不是指令」时，不执行，原句照常保留在记录中；
- 指令句也只保存在 RTC 记录里，不因进入指令模式而自动粘贴到外部应用。

TypeSafe 代理从环境变量 `TYPESAFE_API_KEY` 读取密钥，也支持本地 `config.json` 的 `settings.typesafe.apiKey`。密钥不从浏览器请求体传给第三方。
